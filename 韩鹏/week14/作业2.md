...
---
name: 股票分析器
description: 调用第三方接口 https://api.autostock.cn 获取股票相关数据，并绘制图表进行分析，最终给出历史收益最高的买入卖出时间结论
---

# 解决步骤

## 获取股票信息，可使用的查询接口如下，方法实现参考最后的python代码
...
## 生成显示周和日波动图代码，并截图存入本地路径，最终返回图片地址

### 将图片保存到：
   D:/stock_analyzer/

## 根据生成的波动图，及以下的分析方法，给出建议的买入和卖出时间
...
## 图片路径
D:/stock_analyzer/000001_analysis.png
```
